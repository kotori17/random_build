...
package_linux-zen-git() {
	...
	backup=(etc/mkinitcpio.d/linux-zen-wulan17.conf)
	install=linux-zen-wulan17.install
	...
	install -D -m644 "arch/x86/boot/bzImage" "$pkgdir/boot/vmlinuz-linux-zen-wulan17"
	...
	sed -i "s/_kernel_version=.*/_kernel_version=$_kernver/" \
		"$startdir/linux-zen-wulan17.install"

	msg2 "Installing files for mkinitcpio..."
	install -D -m644 "${srcdir}/linux-zen-wulan17.conf" \
		"$pkgdir/etc/mkinitcpio.d/linux-zen-wulan17.conf"
	
	install -D -m644 "${srcdir}/linux-zen-wulan17.preset" \
		"$pkgdir/etc/mkinitcpio.d/linux-zen-wulan17.preset"
